...
# def train_batch(model, vqvae, surface, criterion):
    with torch.no_grad():
        ...
        # _, _, centers_quantized_test, _, _, encodings_test = vqvae.encode(surface)

    centers_quantized, encodings = sort(centers_quantized, encodings)

    x_logits, y_logits, z_logits, latent_logits = model(centers_quantized, encodings, categories)  ### 这里通过自回归模型来分别预测点坐标以及对应latent的codebook index


    loss_x = criterion(x_logits, centers_quantized[:, :-1, 0])
    loss_y = criterion(y_logits, centers_quantized[:, :-1, 1])
    loss_z = criterion(z_logits, centers_quantized[:, :-1, 2])
